...
어떤 실수 $u \in \mathbb{R}$와 확률변수 $X$에 대해 주어지는 특성함수(ChF) $\phi_X(u)$는 CDF $F_X(x)$의 푸리에-스틸체스 변환(Fourier-Stieltjes Transform; FST)입니다.
...
이 경우 힐베르트 공간의 모든 $f(x)$는 기저 벡터의 선형결합으로 표현될 수 있습니다. 
(힐베르트 공간에서의 함수는 그냥 벡터와 비슷하며 사실 이들은 벡터이나 힐베르트 공간은 무한 차원이며, 어떤 함수의 길이가 유한해서 $\int | f(x) |^2 < \infty$ 일때 힐베르트 공간에 속한다고 하는데 공부를 좀 더 해봐야 할 것 같습니다..)
...
$$
\mathcal{M}_X(u) = \phi_X(-iu) = \int_\mathbb{R} e^{ux}f_X(x)dx
$$
...
$$
\sum_{k=0}^{\infty} {u^k \over k!} \int_\mathbb{R} x^k f_X(x)dx =: \sum_{k=0}^{\infty} \text{moment}_k {u^k \over k!}
$$
...
$$
\zeta_k := \left.\frac{\partial^k \zeta_X(u)}{\partial u^k}\right|_{u=0}
$$
...
$$
f_X(x) = {1 \over 2\pi} \int_\mathbb{R} \phi_X(u) e^{-iux} du
$$
...
$$
f_X(x) = {1 \over 2\pi} \sum_{k=0}^\infty {m_k \over k!} \int_\mathbb{R} (iu)^k e^{-iux} du
$$
...
$$
\delta(x) = {1 \over 2\pi} \int_\mathbb{R} e^{-iux} du
$$

디렉 델타 함수의 $k$-th 미분:

$$
\delta^{(k)}(x) = {d^k \over dx^k} \delta(x) = {1 \over 2\pi} \int_\mathbb{R} (-iu)^k e^{-iux} du
$$
